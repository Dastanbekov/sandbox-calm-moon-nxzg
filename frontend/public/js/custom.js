...
(function($, myApp, window) {
  ...
  //tags array can be dynamically generated with feed or ajax call to ????
  var tags = ['tag1', 'me', 'photo', 'pic'];

  $('#autocomplete').autocomplete({
    lookup: tags,
    onSelect: function(suggestion) {
      buildLabel(suggestion.value);
  ...
  function buildLabel(str) {
    //add new tag to selected list
    $('.selected').append('<span class="label info">' + str + '<a href="#" class="removeme close">&times;</a></span>');

    //clear inpput field afterwards
...
function toggleAndChangeText() {
  ...
  if ($('#ToToggle').css('display') == 'none') {
    $('#aTag').html('Добавить карту');
  }
  else {
    $('#aTag').html('Убрать карту');
  }
}
...
function toggleAndChangeText1() {
  $('#ToToggle1').toggle();
  if ($('#ToToggle1').css('display') == 'none') {
    $('#aTag1').html('Получить контакты');
  }
  else {
    $('#aTag1').html('Скрыть контакты');
  }
}
