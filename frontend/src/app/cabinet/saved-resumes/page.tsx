...
export default function SavedResumesPage() {
  ...
  const fetchItems = async () => {
    try {
      const res = await api.get('/api/resumes/bookmarks/');
  ...
  return (
    <div className="bg-white border border-slate-200/80 rounded-2xl p-6 md:p-8 shadow-xs">
      <div className="border-b border-slate-100 pb-4 mb-6">
        <h2 className="text-xl md:text-2xl font-bold text-slate-900">Сохраненные резюме</h2>
      </div>
      ...
      <div className="flow-root">
        {isLoading ? (
          ...
          <div className="text-center py-12 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
            <p className="text-slate-500 font-medium">У вас нет сохраненных резюме.</p>
          </div>
        ) : (
          <div className="divide-y divide-slate-100">
            {items.map((resume: any) => (
              <div className="py-5 first:pt-0 last:pb-0" key={resume.id}>
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  ...
                  <div className="flex-shrink-0 self-start sm:self-center">
                    <a 
                      href="#" 
                      onClick={(e) => handleRemove(e, resume.id)} 
